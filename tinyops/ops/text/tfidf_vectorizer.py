...
def tfidf_vectorizer(corpus: list[str]) -> Tensor:
    ...
    for term_index in range(vocabulary_size):
        document_frequencies[term_index] = sum(
            1 for document_counts in counts if document_counts[term_index] > 0
        )

    inverse_document_frequencies = [
        math.log((document_count + 1) / (document_frequencies[i] + 1)) + 1
