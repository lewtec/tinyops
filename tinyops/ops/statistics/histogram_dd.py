...
def histogram_dd(
    ...
    if samples.ndim == 1:
        ...
        raise ValueError(f"samples must be 1D or 2D (n_samples, n_dimensions), got shape {samples.shape}")
    ...
    if isinstance(number_of_bins, int):
        ...
        if len(bins_per_dimension) != dimension_count:
            raise ValueError(
                f"number_of_bins length {len(bins_per_dimension)} must match dimension count {dimension_count}"
            )
    ...
    if value_ranges is not None and len(value_ranges) != dimension_count:
        raise ValueError(f"value_ranges length {len(value_ranges)} must match dimension count {dimension_count}")

    if weights is not None:
        if weights.ndim != 1 or weights.shape[0] != sample_count:
            raise ValueError(f"weights must have shape ({sample_count},), got {weights.shape}")
