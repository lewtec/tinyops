...
def two_dimensional_discrete_fourier_transform(complex_image: Tensor) -> Tensor:
    ...
    if complex_image.ndim != 3 or complex_image.shape[-1] != 2:
        raise ValueError(f"complex_image must have shape (H, W, 2), got {complex_image.shape}")
    ...
    return Tensor.stack(
        [discrete_fourier_transform(after_width[:, column_index, :]) for column_index in range(width)],
        dim=1,
    )
