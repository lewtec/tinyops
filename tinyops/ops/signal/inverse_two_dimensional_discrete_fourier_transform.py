...
def inverse_two_dimensional_discrete_fourier_transform(complex_spectrum: Tensor) -> Tensor:
    ...
    if complex_spectrum.ndim != 3 or complex_spectrum.shape[-1] != 2:
        raise ValueError(f"complex_spectrum must have shape (H, W, 2), got {complex_spectrum.shape}")
    ...
    after_width = Tensor.stack(
        [inverse_discrete_fourier_transform(complex_spectrum[row_index]) for row_index in range(height)],
        dim=0,
    )
    return Tensor.stack(
        [inverse_discrete_fourier_transform(after_width[:, column_index, :]) for column_index in range(width)],
        dim=1,
    )
