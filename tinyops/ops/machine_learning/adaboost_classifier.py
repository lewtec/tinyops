...
def adaboost_classifier(
    ...
    one_hot = (estimator_predictions.unsqueeze(-1) == classes.reshape(1, 1, class_count)).cast(dtypes.float32)

    weighted_votes = (one_hot * estimator_weights.reshape(estimator_count, 1, 1) * learning_rate).sum(axis=0)

    predicted_indices = weighted_votes.argmax(axis=1)

    index_one_hot = (predicted_indices.unsqueeze(-1) == Tensor.arange(class_count).reshape(1, class_count)).cast(
        dtypes.float32
    )
