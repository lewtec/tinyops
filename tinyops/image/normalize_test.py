...
from tinyops.image.normalize import NORM_MINMAX, normalize
...
@pytest.mark.parametrize("input_dtype, output_dtype", [(np.float32, dtypes.float32), (np.uint8, dtypes.uint8)])
@assert_one_kernel
def test_normalize_minmax(input_dtype, output_dtype):
    ...
    normalized_image_cv2 = cv2.normalize(
        image, None, alpha, beta, NORM_MINMAX, dtype=cv2.CV_32F if output_dtype == dtypes.float32 else cv2.CV_8U
    )
    ...
    # Compare the results
    assert_close(
        normalized_image_tinyops.cast(dtypes.float32), normalized_image_cv2.astype(np.float32), atol=1, rtol=1e-5
    )
