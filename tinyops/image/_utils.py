...
def _apply_filter_iterative(x: Tensor, kernel: Tensor, scale: float, delta: float) -> Tensor:
    input_dtype = x.dtype
    ...
    if len(orig_shape) == 2: # Grayscale (H, W)
        ...
        padding = ksize // 2
        x_padded = _pad_reflect_101(x_reshaped, (padding, padding, padding, padding))
        ...
        for i in range(C):
            ...
            padding = ksize // 2
            channel_padded = _pad_reflect_101(channel, (padding, padding, padding, padding))
