...
def warp_perspective(src: Tensor, M: Tensor, dsize: tuple[int, int], border_value: float = 0.0) -> Tensor:
    """
    Applies a perspective transformation to an image.

    Args:
        src (Tensor): The input image. Shape: (H, W, C) or (H, W).
        M (Tensor): The 3x3 transformation matrix.
    ...
    # Invert the matrix
    M_inv = inv(M.cast(dtypes.float32))
    ...
    transformed_coords = M_inv.matmul(coords)

    # Normalize the coordinates
    w = transformed_coords[2]
    src_x_orig = transformed_coords[0] / w
    src_y_orig = transformed_coords[1] / w
