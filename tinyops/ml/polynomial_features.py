...
# Limit to prevent DoS via combinatorial explosion
...
def polynomial_features(X: Tensor, degree: int = 2, interaction_only: bool = False, include_bias: bool = True) -> Tensor:
    ...
    # 🛡️ Sentinel: Validate output size before generating combinations.
    # The number of features can grow extremely fast (combinatorially).
    # Iterating over millions of combinations in Python would cause a DoS (hang/timeout).
    ...
    if expected_features > MAX_OUTPUT_FEATURES:
        raise ValueError(f"Polynomial features count {expected_features} exceeds the security limit of {MAX_OUTPUT_FEATURES}.")
