...
def polynomial_features(X: Tensor, degree: int = 2, interaction_only: bool = False, include_bias: bool = True) -> Tensor:
    ...
    # Calculate expected number of output features to prevent DoS
    ...
    if n_output_features > MAX_OUTPUT_FEATURES:
        raise ValueError(f"Too many output features: {n_output_features}. Max allowed is {MAX_OUTPUT_FEATURES}.")
