...
def einsum(subscripts: str, *operands: Tensor) -> Tensor:
    ...
    if "->" in subscripts: input_str, output_str = subscripts.split("->"); explicit = True
    else: input_str, output_str, explicit = subscripts, "", False
    input_subs = input_str.split(",")
    if len(input_subs) != len(operands): raise ValueError("Subscripts mismatch")
    new_operands: List[Tensor] = []; new_input_subs: List[str] = []
    for sub, op in zip(input_subs, operands):
        ...
        while True:
            ...
            dim = op.shape[idxs[0]]
            for i in idxs[1:]:
                if op.shape[i] != dim: raise ValueError(f"Dim mismatch for {c}")
            arange_idx = Tensor.arange(dim)
            indexer = [slice(None)] * op.ndim
            for i in idxs: indexer[i] = arange_idx
            op = op[tuple(indexer)]
            ...
            if is_contiguous:
                 ...
                 new_chars.append(c)
                 for i, char in enumerate(sub):
                     if i not in idxs: new_chars.append(char)
            sub = "".join(new_chars)
        new_operands.append(op)
        new_input_subs.append(sub)
    new_subscripts = ",".join(new_input_subs)
    if explicit: new_subscripts += "->" + output_str
