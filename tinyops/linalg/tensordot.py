...
def tensordot(a: Tensor, b: Tensor, axes: Union[int, Sequence[Union[int, Sequence[int]]]] = 2) -> Tensor:
    """Compute tensor dot product along specified axes."""
    axes_a: List[int]; axes_b: List[int]
    ...
    if len(axes_a) != len(axes_b): raise ValueError("Different number of axes")
    for i, j in zip(axes_a, axes_b):
        if a.shape[i] != b.shape[j]: raise ValueError(f"Shape mismatch: {a.shape[i]} != {b.shape[j]}")
    ...
    prod_free_a = 1
    for i in free_a: prod_free_a *= a.shape[i]
    prod_contract = 1
    for i in axes_a: prod_contract *= a.shape[i]
    prod_free_b = 1
    for i in free_b: prod_free_b *= b.shape[i]
