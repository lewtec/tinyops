...
def test_trace_offset():
    ...
    a = Tensor(a_np)
    assert_close(trace(a, offset=1), np.trace(a_np, offset=1))
    assert_close(trace(a, offset=-1), np.trace(a_np, offset=-1))
    assert_close(trace(a, offset=2), np.trace(a_np, offset=2))
...
def test_trace_axes():
    ...
    # Trace over 1, 2
    assert_close(trace(a, axis1=1, axis2=2), np.trace(a_np, axis1=1, axis2=2))

def test_trace_empty():
    a_np = np.zeros((3, 3), dtype=np.float32)
    a = Tensor(a_np)
    assert_close(trace(a, offset=10), np.trace(a_np, offset=10))
