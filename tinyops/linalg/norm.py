...
def norm(x: Tensor, ord: Union[int, float, str, None] = None, axis: Union[int, Tuple[int, ...], None] = None, keepdims: bool = False) -> Tensor:
    """
    ...
    if axis is None:
        if ndim == 1:
            if ord is None: ord = 2
            axis = 0
        elif ndim == 2:
            if ord is None: ord = 'fro'
            axis = (0, 1)
        else:
             if ord is None:
                 x = x.flatten()
    ...
    if isinstance(axis, (tuple, list)) and len(axis) == 2:
        ...
        # Normalize axes
        if row_axis < 0: row_axis += ndim
        if col_axis < 0: col_axis += ndim
