...
def test_time_mask_deterministic(monkeypatch):
    ...
    masked = time_mask(spectrogram, time_mask_param=4, mask_value=0.0)
...
def test_time_mask_iid(monkeypatch):
    spectrogram = Tensor.ones(2, 5, 10)
    ...
    masked = time_mask(spectrogram, time_mask_param=4, mask_value=0.0, iid_masks=True)
...
def test_time_mask_param_full(monkeypatch):
    ...
    monkeypatch.setattr(Tensor, "rand", lambda *shape: mock_values.pop(0).expand(shape))
    masked = time_mask(spectrogram, time_mask_param=10, mask_value=0.0)
