...
def rand_values_basic():
    return (Tensor(np.full((1, 1, 1), 0.5, dtype=np.float32)), Tensor(np.full((1, 1, 1), 0.2, dtype=np.float32)))
...
def test_freq_mask_basic(spectrogram_basic, rand_values_basic, freq_indices_10):
    ...
    masked = freq_mask(
        spectrogram_basic, freq_mask_param=5, mask_value=0.0, _rand_values=(rand1, rand2), _freq_indices=freq_indices_10
    )
...
def test_freq_mask_deterministic():
    ...
    masked = freq_mask(
        spectrogram, freq_mask_param=4, mask_value=0.0, _rand_values=(rand1, rand2), _freq_indices=freq_indices
    )
...
def test_freq_mask_iid():
    ...
    masked = freq_mask(
        ...
        mask_value=0.0,
        iid_masks=True,
        _rand_values=(rand1, rand2),
        _freq_indices=freq_indices,
    )
...
def test_freq_mask_param_full():
    ...
    masked = freq_mask(
        spectrogram, freq_mask_param=10, mask_value=0.0, _rand_values=(rand1, rand2), _freq_indices=freq_indices
    )
